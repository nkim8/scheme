...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals != nil and vals != nil: 
            while formals.second is not nil and vals.second is not nil:
                child.bindings[formals.first] = vals.first
                formals, vals = formals.second, vals.second 
            child.bindings[formals.first] = vals.first 
            if vals.second is not nil or formals.second is not nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 9A
        fxn = target.first
        env.bindings[fxn] = LambdaProcedure(target.second, expressions.second, env) 
        return fxn  
        # END Question 9A
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN Question 8
    expressions = expressions.second
    return LambdaProcedure(formals, expressions, env)
    # END Question 8
...
def do_and_form(expressions, env):
    ...
    if expressions != nil:
        while expressions.second is not nil:
            if scheme_false(scheme_eval(expressions.first, env)):
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        evaluated = scheme_eval(clause.first, env)
...
def check_formals(formals):
    ...
    def checker(formal):
        if not scheme_symbolp(formal):
            raise SchemeError("invalid parameter")
        if formal in duplicates:
            raise SchemeError("duplicate found")       
    while formals.second is not nil:
        checker(formals.first)
        duplicates.append(formals.first)
...
class MuProcedure(UserDefinedProcedure):
    ...
    # BEGIN Question 17
    def make_call_frame(self, formals, env):
        return env.make_child_frame(self.formals, formals)
    # END Question 17
...
def do_mu_form(expressions, env):
    ...
    # BEGIN Question 17
    expressions = expressions.second
    return MuProcedure(formals, expressions) 
    # END Question 17
